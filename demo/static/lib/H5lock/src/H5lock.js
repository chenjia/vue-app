(function() {
    ...
    H5lock.prototype.drawCle = function(x, y) { // 初始化解锁密码面板 小圆圈
        this.ctx.strokeStyle = '#ddd'; //密码的点点默认的颜色
    ...
    H5lock.prototype.update = function(po) { // 核心变换方法在touchmove时候调用
        ...
        this.drawPoint('#26a2ff'); // 每帧花轨迹
        this.drawStatusPoint('#26a2ff'); // 每帧花轨迹

        this.drawLine('#26a2ff', po, this.lastPoint); // 每帧画圆心
    ...
    H5lock.prototype.storePass = function(psw) { // touchend结束之后对密码和状态的处理
        ...
        if (this.pswObj.step == 1) {
            if (this.checkPass(this.pswObj.fpassword, psw)) {
                ...
                this.drawStatusPoint('#00c853');
                this.drawPoint('#00c853');
                this.drawLine('#00c853', this.lastPoint[this.lastPoint.length - 1], this.lastPoint);
                window.localStorage.setItem('passwordxx', JSON.stringify(this.pswObj.spassword));
                window.localStorage.setItem('chooseType', this.chooseType);
            } else {
                document.getElementById('title').innerHTML = '两次不一致，重新输入';
                document.getElementById('title').style.color = "#ef4f4f";
                this.drawStatusPoint('#ef4f4f');
                this.drawPoint('#ef4f4f');
                this.drawLine('#ef4f4f', this.lastPoint[this.lastPoint.length - 1], this.lastPoint);
            ...
            if (this.checkPass(this.pswObj.spassword, psw)) {
                ...
                this.drawStatusPoint('#00c853'); //小点点外圈高亮
                this.drawPoint('#00c853');
                this.drawLine('#00c853', this.lastPoint[this.lastPoint.length - 1], this.lastPoint); // 每帧画圆心
                ...
                this.drawStatusPoint('#ef4f4f');
                this.drawPoint('#ef4f4f');
                this.drawLine('#ef4f4f', this.lastPoint[this.lastPoint.length - 1], this.lastPoint); // 每帧画圆心

                var title = document.getElementById("title");
                title.style.color = "#ef4f4f";
    ...
    H5lock.prototype.initDom = function() {
        ...
        var str = '<h4 id="title" class="title" style="color:#FFF;">请绘制您的图形密码</h4>' +
            '<a id="updatePassword" style="position: absolute;right: 5px;top: -28px;color:#fff;font-size: 10px;display:none;">重置密码</a>';

        wrap.setAttribute('id', 'h5lock');
        wrap.setAttribute('style', 'position: absolute;top:40px;left:0;right:0;bottom:0;background:url(static/img/lock-bg.jpg) no-repeat;background-size:cover;text-align:center;');
