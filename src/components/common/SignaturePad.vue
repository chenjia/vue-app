<template>
  <div class="pd-sm" :style="{width:w,height:h}" @touchmove.prevent>
    <canvas :id="uid" class="canvas" :data-uid="uid" :disabled="disabled" style="width:100%;height:100%;border-radius: 4px;border:1px dashed #ccc;box-shadow: 0 0 10px #ccc;"></canvas>
    
    <div class="flexable" style="position:absolute;left:0;bottom:10px;width:100%;justify-content: space-around;">
      <mt-button @click="undo()" size="small">
        <i class="fa fa-fw fa-undo"></i>
        <span>撤销</span>
      </mt-button>
      <mt-button @click="clear()" type="danger" size="small">
        <i class="fa fa-fw fa-times"></i>
        <span>清除</span>
      </mt-button>
      <mt-button @click="save()" type="primary" size="small">
...
export default {
  ...
  methods:{
    ...
    getDataURL(format){
      return this.sig.toDataURL(format||'image/png')
    },
    save(format){
      this.$emit('save', this.getDataURL(format))
